...
def train_net(args, config):
    ...
    if CUDA_AVAILABLE and args.dist:
        ...
        if config.TRAIN.OPTIMIZER == 'SGD':
            ...
            raise ValueError(f"Not support optimizer {config.TRAIN.OPTIMIZER}!")
    ...
    if (rank is None) or (rank == 0):
        epoch_end_callbacks = [Checkpoint(model_prefix,
                                          config.CHECKPOINT_FREQUENT)]
    ...
    batch_end_callbacks = [
        Speedometer(batch_size, config.LOG_FREQUENT,
    ...
    if config.TRAIN.LR_SCHEDULE == 'plateau':
        ...
        lr_scheduler = WarmupMultiStepLR(
            optimizer, milestones=lr_iters, gamma=config.TRAIN.LR_FACTOR,
        ...
        raise ValueError(f"Not support lr schedule: {config.TRAIN.LR_SCHEDULE}!")
    ...
    train(
        model, optimizer, lr_scheduler, train_loader, train_sampler,
        train_metrics, config.TRAIN.BEGIN_EPOCH, config.TRAIN.END_EPOCH,
        logger,
        rank=rank,
